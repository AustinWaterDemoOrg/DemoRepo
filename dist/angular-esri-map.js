...
(function(angular) {
    ...
    angular.module('esri.map').directive('esriMapLayer', function ($q) {
        // this object will tell angular how our directive behaves
        return {
            // only allow esriMapLayer to be used as an element (<esri-map-layer>)
            restrict: 'E',

            // require the esriMapLayer to have its own controller as well an esriMap controller
            // you can access these controllers in the link function
            require: ['esriMapLayer', '^esriMap'],
            ...
            controller: function ($scope, $element, $attrs) {
                ...
                require([
                    'esri/layers/ArcGISDynamicMapServiceLayer'], function (MapLayer) {
                    var layer = new MapLayer($attrs.url);
...
(function(angular) {
    ...
    /**
     * @ngdoc directive
     * @name esriApp.directive:esriLocateButton
     * @description
     * # esriLocateButton
     */
    angular.module('esri.map')
      .directive('esriLocateButton', function ($document, $q) {
        return {
          //run last
          priority: -10,
          scope:{},
          ...
          link: function(scope, element, attrs, controllers){
            ...
            var mapController = controllers[0];
            var targetId = attrs.targetId || attrs.id;
            var locationButtonDeferred = $q.defer();

            require(['esri/dijit/LocateButton', 'dijit/registry'], function (LocateButton, registry) {
              mapController.getMap().then(function(map) {
                var opts = {
                    map: map
                };
                ...
                // NOTE: need to come up w/ a way to that is not based on id
                // or handle destroy at end of this view's lifecyle
                var locateButton = registry.byId(targetId);
                if (locateButton) {
                  locateButton.destroyRecursive();
                }
                locateButton = new LocateButton(opts, targetId);
                locateButton.startup();
    
                locationButtonDeferred.resolve(locateButton);
              });
            });
          }
        };
